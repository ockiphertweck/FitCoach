...
export async function requireAnthropicClient(userId: string): Promise<Anthropic> {
  const client = await getAnthropicClient(userId)
  if (!client) throw new Error("No Claude API key configured. Add one in Settings.")
  return client
}
