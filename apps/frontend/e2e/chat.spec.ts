...
const HISTORY_MESSAGES = [
  ...
  },
  {
    ...
    role: "assistant",
    content:
      "Your training load looks solid. You hit **3 sessions** this week.",
...
test.describe("AI Coach chat", () => {
  test.describe("Empty state", () => {
    test("shows empty state placeholder when no history", async ({
      authedPage,
    }) => {
    ...
    test("Clear button hidden when no messages", async ({ authedPage }) => {
      ...
      await authedPage.goto("/coach");
      await expect(
        authedPage.getByRole("button", { name: "Clear" }),
      ).not.toBeVisible();
    });
  });
  ...
  test.describe("History loading", () => {
    test("renders existing chat messages on load", async ({ authedPage }) => {
      ...
      await authedPage.goto("/coach");
      await expect(
        authedPage.getByText("How was my training this week?"),
      ).toBeVisible();
      await expect(
        authedPage.getByText(/Your training load looks solid/),
      ).toBeVisible();
    });
    ...
    test("renders markdown in assistant messages", async ({ authedPage }) => {
      ...
      await expect(
        authedPage.locator("strong").filter({ hasText: "3 sessions" }),
      ).toBeVisible();
    });
  ...
  test.describe("Sending messages", () => {
    ...
    test("send button enabled when input has text", async ({ authedPage }) => {
      await authedPage.goto("/coach");
      await authedPage
        .getByPlaceholder(/Ask your coach/)
        .fill("How should I train?");
      await expect(
        authedPage.locator("textarea").locator("xpath=..").locator("button"),
      ).toBeEnabled();
    });

    test("streams assistant response after sending message", async ({
      authedPage,
    }) => {
      ...
      await mockGet(authedPage, /\/ai\/chat\/history\?sessionId=/, {
        messages: [
          ...
          },
          {
            ...
            role: "assistant",
            content:
              "Based on your recent load, I recommend an easy run tomorrow.",
            createdAt: new Date().toISOString(),
          },
        ],
      });
      ...
      await authedPage
        .getByPlaceholder(/Ask your coach/)
        .fill("What should I do tomorrow?");
      await authedPage
        .locator("textarea")
        .locator("xpath=..")
        .locator("button")
    ...
    test("Shift+Enter creates newline without submitting", async ({
      authedPage,
    }) => {
      let called = false;
      await authedPage.route(`${API_BASE}/ai/chat`, (route) => {
  ...
  test.describe("Clear history", () => {
    test("clicking clear removes all messages", async ({ authedPage }) => {
      ...
      await mockGet(authedPage, "/ai/chat/sessions", {
      ...
      await authedPage.route(/\/ai\/chat\/history\?sessionId=/, (route) => {
        ...
        route.fulfill({
          ...
          body: JSON.stringify({
            messages: historyCallCount === 1 ? HISTORY_MESSAGES : [],
          }),
        });
      });
...
test.describe("AI Coach — retry on failure", () => {
  ...
  test("retry button resends the original message", async ({ authedPage }) => {
    ...
    await authedPage.route(/\/ai\/chat\/history\?sessionId=/, (route) => {
      ...
      if (historyCallCount <= 1) {
        ...
        // After successful retry: return persisted messages
        route.fulfill({
          status: 200,
          contentType: "application/json",
          body: JSON.stringify({
            messages: [
              {
                id: "m1",
                role: "user",
                content: "Am I recovered?",
                createdAt: new Date().toISOString(),
              },
              {
                id: "m2",
                role: "assistant",
                content: "You are recovered!",
                createdAt: new Date().toISOString(),
              },
            ],
          }),
  ...
  test("retry button disappears after successful retry", async ({
    authedPage,
  }) => {
    await setupChat(authedPage);
