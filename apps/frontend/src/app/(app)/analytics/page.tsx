  ...
  return (
    <div className="space-y-5">
      <div>
        ...
        <p className="text-muted-foreground text-sm mt-0.5">AI-generated analysis of your training week</p>
      </div>
      ...
      <Card>
        ...
        <CardContent className="px-5 pb-5 space-y-5">
          <Button onClick={() => reportMutation.mutate()} disabled={reportMutation.isPending} size="sm">
          ...
          {reportMutation.data && (
            <div className="space-y-5">
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                {[
                  { label: "Distance", value: `${(reportMutation.data.metrics.totalDistance / 1000).toFixed(1)} km` },
                  { label: "Duration", value: `${Math.round(reportMutation.data.metrics.totalDuration / 60)} min` },
                  { label: "Sessions", value: String(reportMutation.data.metrics.sessions) },
                  { label: "Avg HR", value: reportMutation.data.metrics.avgHR ? `${reportMutation.data.metrics.avgHR} bpm` : "N/A" },
                  { label: "ATL", value: String(reportMutation.data.metrics.atl) },
                  { label: "CTL", value: String(reportMutation.data.metrics.ctl) },
                ].map(({ label, value }) => (
                  <div key={label} className="rounded-xl bg-white/30 border border-white/50 px-4 py-3">
